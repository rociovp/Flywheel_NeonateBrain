...
def exec_mrview(output_folder,subject_id,t1_file,file_name,pattern_file_name):
    ...
    comand_mrview_saggital = 'mrview "' + t1_file + '" -plane 0 -imagevisible 1 -overlay.load "'+ file_name + '" -overlay.opacity 0.5 -overlay.colourmap 255 -capture.folder '+output_folder +' -capture.prefix '+str(subject_id)+'_'+str(pattern_file_name.replace("t1.nii.gz","")) +'_Saggital_ -capture.grab -exit'

    print(comand_mrview_saggital )
    if (t1_file!='None') & (file_name!='None'):
        os.system(comand_mrview_saggital)
    else:
        print('No file')

    comand_mrview_coronal = 'mrview "' + t1_file + '" -plane 1 -imagevisible 1 -overlay.load "'+ file_name + '" -overlay.opacity 0.7 -overlay.colourmap 255 -capture.folder '+output_folder +' -capture.prefix '+str(subject_id)+'_'+str(pattern_file_name.replace("t1.nii.gz","")) +'_Coronal_ -capture.grab -exit'
...
file_name_list=[ 'brainmask.nii.gz']
...
for i in tqdm(folder_list): # For all the folders 
    ...
    for pattern_file_name in file_name_list: # Now we iterate for all the tracts in the subject id
        ...
        output_folder= '/Users/rociovp/Desktop/montage_test/'
